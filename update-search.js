...
// DEINE EXAKTEN, SCHARFEN KRITIK-TEXTE – 1:1 wie früher gewünscht
function generiereKritik(text, url = "") {
  ...
  if ((lower.includes("gutachten") || lower.includes("gutachter")) && (lower.includes("ablehnen") || lower.includes("mangelhaft") || lower.includes("beeinflussen") || lower.includes("kritisch"))) {
    return "Kritisch: Hohe Hürden und mangelhafte Gutachten als Ablehnungs-Tür – impliziert Manipulation von Gutachten (§ 153 StGB).";
  }
  if (lower.includes("kindeswohl") && (lower.includes("argument") || lower.includes("schaden") || lower.includes("nicht geeignet") || lower.includes("nachteil"))) {
    return "Kritisch: Direkter Rat zur Verhinderung durch ‚Kindeswohl-Argumente' – impliziert selektive Darstellung, Grenze zu § 153 StGB (falsche Aussage).";
  }
  ...
  return "Kritisch: Indirekte Strategie gegen das Wechselmodell erkennbar (z. B. Eskalation oder fehlende Kooperation als Argument)";
}
...
async function main() {
  ...
  for (const begriff of SUCHBEGRIFFE) {
    ...
    for (const item of ergebnisse) {
      ...
      const kritik = generiereKritik(inhalt.text, url);
    ...
    await new Promise(r => setTimeout(r, 3500)); // Höflichkeitspause
  }
